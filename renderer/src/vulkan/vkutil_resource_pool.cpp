...
{
ResourceFactory<CommandBuffer>::ResourceFactory(rcp<VulkanContext> vk, uint32_t queueFamilyIndex) :
    m_vk(std::move(vk))
{
    ...
    VK_CHECK(
        m_vk->CreateCommandPool(m_vk->device, &commandPoolCreateInfo, nullptr, &m_vkCommandPool));
}
...
CommandBuffer::CommandBuffer(rcp<VulkanContext> vk, VkCommandPool vkCommandPool) :
    m_vk(std::move(vk)), m_vkCommandPool(vkCommandPool)
{
    ...
    VK_CHECK(
        m_vk->AllocateCommandBuffers(m_vk->device, &commandBufferAllocateInfo, &m_vkCommandBuffer));
}

CommandBuffer::~CommandBuffer()
{
    m_vk->FreeCommandBuffers(m_vk->device, m_vkCommandPool, 1, &m_vkCommandBuffer);
}
...
Semaphore::Semaphore(rcp<VulkanContext> vk) : m_vk(std::move(vk))
{
    ...
    VK_CHECK(m_vk->CreateSemaphore(m_vk->device, &semaphoreCreateInfo, nullptr, &m_vkSemaphore));
}

Semaphore::~Semaphore() { m_vk->DestroySemaphore(m_vk->device, m_vkSemaphore, nullptr); }
