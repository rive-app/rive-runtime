...
// Combined D3D11 + D3D12 Texture implementation for Windows.
...
void Texture::upload(const TextureDataDesc& data)
{
    ...
    {
        // D3D12 path — staging upload buffer + copy command.
        ...
        // (see D3D12CalcSubresource). See sibling fix in
        // ore_texture_d3d12.cpp.
        ...
        assert(SUCCEEDED(hr) &&
               "Texture::upload: staging buffer creation failed");
        ...
        ctx->m_d3dUploadCmdList->CopyTextureRegion(&dst_loc,
                                                   data.x,
                                                   data.y,
                                                   data.z,
                                                   &src_loc,
        ...
        // Transition to PIXEL_SHADER_RESOURCE so it's ready to sample.
        {
            D3D12_RESOURCE_BARRIER barrier = {};
            barrier.Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
            barrier.Transition.pResource = m_d3dTexture.Get();
        ...
        // Keep the staging buffer alive until the GPU is done.
