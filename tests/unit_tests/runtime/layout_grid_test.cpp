...
// (gen_grid_min_content): four 400x400 containers, each holding one fill child.
// These are hand-authored rather than exported, so names survive.
static rive::LayoutComponent* namedLayout(rive::ArtboardInstance* artboard,
                                          const char* name)
{
    for (auto layout : artboard->find<rive::LayoutComponent>())
    {
        if (!layout->is<rive::Artboard>() && layout->name() == name)
        {
            return layout;
        }
    }
...
// contribution. A child that can reflow answers "one chip wide", so the track
// stays the container and the child wraps inside it. Measured at max-content
// instead -- the item's width when nothing squeezes it -- the floor was the
// whole unwrapped row and every fill child in the track grew with it.
TEST_CASE("an intrinsic track floors at min-content, not max-content",
          "[layoutgrid]")
{
    auto file = ReadRiveFile("assets/layout/grid_min_content.riv");
    auto artboard = file->artboardNamed("ReflowGrid");
    REQUIRE(artboard != nullptr);
    artboard->advance(0.0f);

    auto* container = namedLayout(artboard.get(), "Container");
    auto* child = namedLayout(artboard.get(), "Child");
    ...
    // ...and it really did reflow: four 150px chips in a 400 wide row wrap two
    // per line, so the last one sits on a second line rather than off the end.
    std::vector<rive::LayoutComponent*> chips;
    for (auto layout : artboard->find<rive::LayoutComponent>())
    {
        if (layout->parent() == child)
        {
            chips.push_back(layout);
        }
    }
    REQUIRE(chips.size() == 4);
    std::sort(chips.begin(), chips.end(), [](auto a, auto b) {
        return a->layoutY() < b->layoutY();
    });
    CHECK(chips.front()->layoutY() == 0.0f);
    CHECK(chips.back()->layoutY() > 0.0f);
}

// The other half of that: when the content genuinely cannot shrink, its
// min-content really is the full width, the track grows and the child overruns
// -- which is what a browser does too. Guards the fix against flooring every
// track at nothing, which would "fix" the overrun by breaking this.
TEST_CASE("content that cannot shrink still grows an intrinsic track",
          "[layoutgrid]")
{
    auto file = ReadRiveFile("assets/layout/grid_min_content.riv");
    auto artboard = file->artboardNamed("RigidGrid");
    REQUIRE(artboard != nullptr);
    artboard->advance(0.0f);
    ...
    CHECK(container->layoutWidth() == 400.0f);
    CHECK(child->layoutWidth() == 600.0f);
}

// The flip side of "cannot shrink": a wrapping row has no irreducible width at
// all. It collapses and lets an oversized item overflow its line, so its
// min-content is 0 and the track keeps the container -- and a browser agrees
// (min-content of a wrapping flex container measures 0 there too). Worth
// pinning: wrap is what decides this, not the item's own rigidity.
TEST_CASE("a wrapping child gives an intrinsic track nothing to floor at",
          "[layoutgrid]")
{
    auto file = ReadRiveFile("assets/layout/grid_min_content.riv");
    auto artboard = file->artboardNamed("WrapRigidGrid");
    REQUIRE(artboard != nullptr);
    artboard->advance(0.0f);
...
// minmax(0, 1fr): an authored minimum is not intrinsic, so no content floor is
// distributed into the track and the same unshrinkable child is clamped to the
// container. Before, the flexible-track step ignored the track's min sizing
// function entirely and a 0 minimum did nothing.
TEST_CASE("an authored zero minimum removes the content floor", "[layoutgrid]")
{
    auto file = ReadRiveFile("assets/layout/grid_min_content.riv");
    auto artboard = file->artboardNamed("MinMaxGrid");
    REQUIRE(artboard != nullptr);
    artboard->advance(0.0f);
