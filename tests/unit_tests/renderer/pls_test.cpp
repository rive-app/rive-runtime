...
TEST_CASE("FindTransformedArea", "[pls]")
{
    ...
    CHECK(gpu::FindTransformedArea(unitSquare, Mat2D::fromRotation(math::PI / 4)) == Approx(1.f));
    CHECK(gpu::FindTransformedArea(unitSquare, Mat2D::fromRotation(math::PI / 8).scale({2, 2})) ==
          Approx(4.f));
    CHECK(gpu::FindTransformedArea(unitSquare, Mat2D::fromRotation(math::PI / 16).scale({2, 1})) ==
          Approx(2.f));
