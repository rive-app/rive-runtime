...
// Make sure gradients don't go NaN-tastic when they have co-located (degenerate) stops, and that
// they render as specified when stops are co-located.
...
public:
    ...
    void onDraw(rive::Renderer* renderer) override
    {
        ...
        // Stuff a red stop in the middle and ensure no red shows up in the drawing.
        ...
        for (float stop : {.0f, .5f, 1.f})
        {
            ...
            paint->shader(factory->makeLinearGradient(0, 0, 150, 150, colors2, stops, 2));
            renderer->drawPath(rect.get(), paint.get());

            renderer->translate(200, 0);
            paint->shader(factory->makeRadialGradient(75, 75, 75, colors2, stops, 2));
            renderer->drawPath(rect.get(), paint.get());

            renderer->translate(200, 0);
            paint->shader(factory->makeLinearGradient(0, 0, 150, 150, colors3, stops, 3));
            renderer->drawPath(rect.get(), paint.get());

            renderer->translate(200, 0);
            paint->shader(factory->makeRadialGradient(75, 75, 75, colors3, stops, 3));
            renderer->drawPath(rect.get(), paint.get());
