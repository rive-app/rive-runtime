...
static void add_star(Path& path, int count, float dir)
{
    ...
    else
    {
        count >>= 1;
        make_star(path, count, 0, dir);
        make_star(path, count, rive::math::PI, 1); // always wind one on the 2nd contour
    }
}

static std::string fillrule_to_name(rive::FillRule fr)
{
    return std::string("poly_") + (fr == rive::FillRule::nonZero ? "nonZero" : "evenOdd");
}
...
public:
    PolyGM(rive::FillRule fr) : GM(480, 480, fillrule_to_name(fr).c_str()), m_FillRule(fr) {}

    void onDraw(rive::Renderer* ren) override
    {
        auto draw = [this, ren](float tx, float ty, rive::ColorInt c, int pts, float dir) {
            Path path;
