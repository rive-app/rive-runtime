...
static void draw_clipped_flower(Renderer* renderer, FillRule fillType)
{
    ...
    for (int y = 0; y < kGridCount; ++y)
    {
        clip.addRect(AABB(0, y * kCellSize, kSize, (y + 1) * kCellSize),
                     (y & 1) ? rivegm::PathDirection::cw : rivegm::PathDirection::ccw);
    }
    for (int x = 0; x < kGridCount; ++x)
    {
        clip.addRect(AABB(x * kCellSize, 0, (x + 1) * kCellSize, kSize),
                     (x & 1) ? rivegm::PathDirection::cw : rivegm::PathDirection::ccw);
    }
...
DEF_SIMPLE_GM_WITH_CLEAR_COLOR(largeclippedpath_winding, 0xff00ffff, kSize, kSize, renderer)
{
    draw_clipped_flower(renderer, FillRule::nonZero);
}

DEF_SIMPLE_GM_WITH_CLEAR_COLOR(largeclippedpath_evenodd, 0xff00ffff, kSize, kSize, renderer)
{
