...
// Same scene as ore_depth, but the nearest quad is drawn with an always
// compare and depth writes on. The later lessEqual quads only stay behind it
// if that write landed, so the golden must match ore_depth exactly.
...
class OreDepthWriteAlwaysGM : public GM
{
public:
    OreDepthWriteAlwaysGM() : GM(256, 256) {}
    ...
    defined(ORE_BACKEND_RHI)
        ...
        bufDesc.label = "ore_depth_write_always_vbo";
        ...
        pipeDesc.label = "ore_depth_write_always_test";
        auto pipeline = ctx.makePipeline(pipeDesc);
        pipeDesc.depthStencil.depthCompare = CompareFunction::always;
        pipeDesc.label = "ore_depth_write_always";
        auto pipelineAlways = ctx.makePipeline(pipeDesc);
        ...
        rpDesc.label = "ore_depth_write_always_pass";

        auto pass = ctx.beginRenderPass(rpDesc);
        pass->setPipeline(pipelineAlways.get());
...
GMREGISTER(ore_depth_write_always, return new OreDepthWriteAlwaysGM())
