...
/*
 * UnrealTestingWindow is the testing window implementation for rhi intended to
 * be used with gms unit testing. This is assumed to be on the Render thread. If
 * used on the Game thread it will cause unexpected behaviour.
 */

class UnrealTestingWindow : public TestingWindow
{
public:
    UnrealTestingWindow(UObject* WorldContextObject,
                        IRiveRenderer* RenderTarget);
...
private:
    // we don't reference count this here because a UObject owns this class and
    // the render target
    IRiveRenderer* RiveRenderer;

    rive::gpu::RenderContext* RenderContext;
    rive::rcp<rive::gpu::RenderTarget> RenderTarget;
    // I am not allowed to both render to and read from the same texture, so i'm
    // going to copy to this one and read from here
